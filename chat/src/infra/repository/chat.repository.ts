...
class ChatRepository implements IChatRepository {
  ...
  async createMessage(messageDTO: MessageAttrs): Promise<MessageAttrs> {
  ...
  async findConversation(
    ...
    console.log(
      "find Conversation from char repo",
      sender,
      receiver,
      context,
      companyId,
  ...
  async getConversation(queryParams: {
    userId?: string;
    companyId?: string;
    context: string;
  }) {
          ...
          pipeline: [
            {
              $match: {
                ...
                ...(queryParams.userId && {
                  sender: {
                    $ne: new mongoose.Types.ObjectId(queryParams.userId),
                  },
                  "readBy.user": {
                    $not: {
                      $eq: new mongoose.Types.ObjectId(queryParams.userId),
                    },
                  },
                }),
                status: { $ne: "read" },
  ...
  async readMessage(
    ...
    await Message.updateMany(
      { conversation: conversationId, sender },
      {
        $set: { status },
      }
    );
  ...
  async deliverAll(receiver: string) {
    try {
      ...
      await Message.updateMany(
        { receiver, status: { $ne: MESSAGE_STATUS.READ } },
        {
