...
import { TokenData } from "@domain/entities/interfaces";
...
class Token {
  static generateJwtToken(
    payload: TokenData,
    option: { createAdminToken: boolean } = { createAdminToken: false }
  ) {
    const { createAdminToken: admin } = option!;
    const accessSecrect = admin
      ? process.env.ADMIN_SECRECT!
      : process.env.TOKEN_SECRECT!;
    const refreshSecrect = admin
