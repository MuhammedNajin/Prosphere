...
const ApplicationItem: React.FC<{ application: Application }> = ({
  ...
  return (
    <div
      className={`flex flex-wrap gap-10 justify-between items-center p-6 w-full rounded-lg ${
        application.company === "Udacity" ? "bg-white" : "bg-slate-50"
    ...
    >
      ...
      <div className="flex flex-col justify-center items-start self-stretch my-auto text-sm font-semibold whitespace-nowrap w-[117px]">
        <div
          className={`gap-2 self-stretch px-2.5 py-1.5 border border-solid rounded-[80px] ${application.statusColor}`}
        >
          {application.status}
        </div>
      </div>
