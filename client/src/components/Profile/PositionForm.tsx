...
function PositionForm({ position, onClose }: PositionForm = { position: null, onClose: () => {} }) {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      locationType: (position?.locationType as "on-site" | "hybrid" | "remote") || "on-site",
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      toast({
        description: <SuccessMessage message='Experience updated successfully'/>,
    ...
    onError: (error: AxiosError<ApiErrorResponse>) => {
      ...
      toast({
        description: <ErrorMessage message='Failed to update position. Please try again.' />,
        className: 'bg-red-500 text-white',
      });
    }
  });
