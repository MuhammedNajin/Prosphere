...
class JobApi {
   ...
   static postJob = async ({ formData, id }) => {
        ...
        return await this.axios.post('/api/v1/job/company/jobs', {...formData, companyId: id, salary: { to: maxSalary, from: minSalary, status: true }}, {
          headers: {
   ...
   static updateJob = async ({ formData, companyId, id }) => {
    ...
    delete formData.maxSalary;
    delete formData.minSalary;
       return this.axios.post(`/api/v1/job/${id}`, {...formData, companyId , salary: { to: maxSalary, from: minSalary, status: true }});
   }
   ...
   console.log("filter", filter, location)
      const queryParams = new URLSearchParams({
        ...
        ...(filter.employment?.length && { employment: filter?.employment?.join(',') }),
        ...(filter.jobLocation?.length && { jobLocation: filter?.jobLocation?.join(',') })
      });
   ...
   static getComment = async ({ jobId }: { jobId: string}) => {
    ...
    try {
      const response = await this.axios.get(`api/v1/job/comment/?jobId=${jobId}`);
